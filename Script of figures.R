...
pHGreenhouse <- read_excel("Ph_greenhouse.xls")
DWGreenhouse <- read_excel("DW_greenhouse.xls")

summary(DWGreenhouse)
...
pHGreenhouse$Hybrid =as.factor(pHGreenhouse$Hybrid)
pHGreenhouse$Treat =as.factor(pHGreenhouse$Treat)
pHGreenhouse$Condition =as.factor(pHGreenhouse$Condition)
pHGreenhouse$Condition = factor(pHGreenhouse$Condition, levels =c("Control soil", "ASS soil"))
pHGreenhouse$Treat = factor(pHGreenhouse$Treat, levels =c("Non-fert", "U","AS"))

DWGreenhouse$Hybrid =as.factor(DWGreenhouse$Hybrid)
DWGreenhouse$Treatment =as.factor(DWGreenhouse$Treatment)
DWGreenhouse$Condition =as.factor(DWGreenhouse$Condition)
DWGreenhouse$Condition = factor(DWGreenhouse$Condition, levels =c("Control soil", "ASS soil"))
DWGreenhouse$Treatment = factor(DWGreenhouse$Treatment, levels =c("Non-fert", "U","AS"))
...
Figure_1b <- pHGreenhouse
Figure_1b %>%  
  ggplot() +
  aes(x=Treat, y=pH, col=Condition)+
  geom_boxplot() + labs(x = "Treatments", y = "Soil pH")+  facet_grid(Condition~Hybrid)+scale_color_manual(values = c("dark grey", "black")) + theme_bw()+  scale_y_continuous(limits = c(5, 11))+ 
  stat_compare_means(comparisons = my_comparisons, label = "p.signif", method = 'wilcox.test', ref.group = "Non-fert", size = 2.5)
...
Figure_2b <- DWGreenhouse
Figure_2b %>%  
  ggplot() +
  aes(x=Treatment, y=DW, col=Condition)+
  geom_boxplot() + labs(x = "Treatments", y = "Shoot dry weight g/plant")+  facet_grid(Condition~Hybrid)+scale_color_manual(values = c("dark grey", "black")) + theme_bw() +  scale_y_continuous(limits = NULL)+
